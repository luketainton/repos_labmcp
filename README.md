...
Unified [Model Context Protocol](https://modelcontextprotocol.io/) server for a home lab. It currently exposes Gitea, Pocket ID, n8n, Shlink, Pushover, and Action1 through one FastMCP process.
...
MCP_SERVICE_GROUPS={"gitea":["mcp-gitea"],"pocket_id":["mcp-pocket-id"],"pangolin":["mcp-pangolin"],"shlink":["mcp-shlink"]}
```

Membership of any group listed for a service permits access to that service's tools. The Gitea tools use the `gitea` service key; Pocket ID tools use `pocket_id`; Pangolin tools use `pangolin`; and Shlink tools use `shlink`. Once `MCP_SERVICE_GROUPS` is non-empty, any omitted service is denied. Leave it as `{}` only when every authenticated user should have access to all services.
...
MCP_SERVICE_SCOPES={"gitea":["labmcp:gitea"],"pocket_id":["labmcp:pocket-id"],"n8n":["labmcp:n8n"],"pangolin":["labmcp:pangolin"],"shlink":["labmcp:shlink"],"action1":["labmcp:action1"]}
```
