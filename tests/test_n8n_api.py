...
def test_parse_operations_rejects_invalid_or_duplicate_documents() -> None:
    ...
    with pytest.raises(ValueError, match="no supported operations"):
        parse_operations({"paths": {"/upload": {"post": {"operationId": "upload", "requestBody": {"content": {"multipart/form-data": {"schema": {"type": "binary"}}}}}}}})
...
async def test_call_operation_rejects_invalid_request_shapes() -> None:
    client = FakeClient()
    operations = parse_operations({"paths": {"/workflows/{id}": {"post": {"operationId": "update", "requestBody": {"content": {"application/json": {}}}}}}})
    ...
    with pytest.raises(ValueError, match="either body or form"):
        await call_operation(client, "update", operations=operations, path_params={"id": "1"}, body={}, form={})
    with pytest.raises(ValueError, match="requires a JSON body"):
        await call_operation(client, "update", operations=operations, path_params={"id": "1"}, form={})
...
async def test_call_operation_requires_form_data_for_form_operations() -> None:
    client = FakeClient()
    operations = parse_operations({"paths": {"/import": {"post": {"operationId": "import", "requestBody": {"content": {"application/x-www-form-urlencoded": {}}}}}}})
...
@pytest.mark.asyncio
async def test_get_operations_parses_and_caches_yaml_specification(monkeypatch: pytest.MonkeyPatch) -> None:
    client = FakeClient()
...
@pytest.mark.asyncio
async def test_get_operations_accepts_string_and_rejects_invalid_documents(monkeypatch: pytest.MonkeyPatch) -> None:
    client = FakeClient()
