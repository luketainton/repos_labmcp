...
def test_service_clients_use_service_specific_auth_headers():
    settings = SimpleNamespace(
        ...
        meraki_url="https://api.meraki.example",
        meraki_dashboard_api_key=SimpleNamespace(get_secret_value=lambda: "meraki-key"),
    ...
    assert (meraki.auth_header, meraki.auth_prefix, meraki.token) == (
        "Authorization",
        "Bearer",
        "meraki-key",
    )
