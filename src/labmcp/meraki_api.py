"""Dynamic access to every supported operation in Meraki's OpenAPI document."""
...
DEFAULT_OPENAPI_URL = "https://raw.githubusercontent.com/meraki/openapi/master/openapi/spec3.json"
_ALLOWED_METHODS = {"get", "post", "put", "patch", "delete"}
_OPERATIONS_CACHE: dict[str, dict[str, "MerakiOperation"]] = {}


@dataclass(frozen=True)
class MerakiOperation:
    method: str
    path: str
    encoding: str
...
) -> dict[str, MerakiOperation]:
    """Parse Meraki's OpenAPI 3 document into safe, callable operation metadata."""
    paths = specification.get("paths")
    if not isinstance(paths, Mapping):
        raise ValueError("Meraki OpenAPI document does not contain paths.")

    prefix = "/" + api_path.strip("/") if api_path.strip("/") else ""
    operations: dict[str, MerakiOperation] = {}
    for route, methods in paths.items():
        ...
        for method, details in methods.items():
            ...
            if operation_id in operations:
                raise ValueError(f"Duplicate Meraki OpenAPI operation ID: {operation_id}")
            encoding = _request_encoding(details)
            if encoding == "binary":
                continue
            operations[operation_id] = MerakiOperation(
                method.upper(), f"{prefix}/{route.lstrip('/')}", encoding
            )
    if not operations:
        raise ValueError("Meraki OpenAPI document contains no supported operations.")
    return operations
...
async def call_operation(
    ...
    operation_name: str,
    *,
    operations: Mapping[str, MerakiOperation],
    path_params: Mapping[str, str] | None = None,
    query: Mapping[str, Any] | None = None,
    body: Any = None,
    form: Mapping[str, Any] | None = None,
) -> Any:
    """Call one documented Meraki operation after validating its route."""
    operation = operations.get(operation_name)
    if operation is None:
        raise ValueError(f"Unknown Meraki operation: {operation_name}")
...
class MerakiOperationProvider(Provider):
    """Expose one MCP tool for every supported Meraki Dashboard API operation."""
    ...
    async def _list_tools(self) -> Sequence[Tool]:
        ...
        operations = await _get_operations(self._openapi_url, self._api_path, self._timeout)
        cache_key = f"{client.base_url}|{self._openapi_url}|{self._api_path}"
...
def _make_operation_tool(
    ...
    tool_name = f"meraki_{_snake_case(operation_name)}"
    execute.__name__ = tool_name
    description = (
        f"Meraki {operation.method} {operation.path}. "
...
async def _get_operations(
    openapi_url: str, api_path: str, timeout: float
) -> dict[str, MerakiOperation]:
    cache_key = f"{openapi_url}|{api_path}"
    if cache_key not in _OPERATIONS_CACHE:
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.get(openapi_url, headers={"Accept": "application/json"})
        try:
            response.raise_for_status()
            specification = response.json()
        except (httpx.HTTPStatusError, ValueError) as exc:
            raise ValueError("Meraki OpenAPI source did not return a valid JSON document.") from exc
        if not isinstance(specification, Mapping):
            raise ValueError("Meraki OpenAPI source did not return a JSON object.")
